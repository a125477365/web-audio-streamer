...
app.get('/api/local/browse', (req, res) => {
 ...
 res.json({ success: true, directories, path: targetPath, parent: targetPath !== '/' ? path.dirname(targetPath) : null, platform: process.platform });
 } catch (error) {
 res.status(500).json({ success: false, error: error.message });
 }
});
...
app.get('/api/online/search', async (req, res) => {
 ...
 res.json({ success: false, error: error.message, hint: '请把以下内容发给 OpenClaw 进行修复：', fixMessage: 'Web Audio Streamer 搜索失败，请自动帮我修复。' });
 }
});

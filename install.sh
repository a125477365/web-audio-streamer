...
echo " 双向 Skill for Hermes/OpenClaw"
echo "========================================="
echo -e "${NC}"
...
check_hermes() {
 ...
 echo ""
 echo -e "${BLUE}检查 Hermes Agent 集成...${NC}"
 
 if [ -x "$HERMES_PYTHON" ] && [ -f "$HERMES_CLI" ]; then
 echo -e "${GREEN}[√] Hermes CLI 可用${NC}"
 echo " Python: $HERMES_PYTHON"
 echo " CLI: $HERMES_CLI"
 echo -e "${GREEN}[√] 首次启动将自动获取音源${NC}"
 else
 echo -e "${YELLOW}[!] Hermes CLI 不可用${NC}"
 echo -e "${YELLOW} 如需自动获取音源，请先安装 Hermes Agent${NC}"
 echo -e "${YELLOW} 或手动配置音源 API 地址${NC}"
 fi
}
...
# 创建 Hermes Skill 配置
create_hermes_skill() {
 SKILL_DIR="$HOME/.hermes/skills/web-audio-streamer"
 
 if [ -d "$HOME/.hermes" ]; then
 echo ""
 echo -e "${BLUE}[→] 创建 Hermes Skill 配置...${NC}"
 mkdir -p "$SKILL_DIR"
...
音乐播放系统，支持本地音乐、在线搜索、网络电台。

## 使用方法

### 播放音乐
```
curl "http://localhost:3010/api/online/search?q=歌曲名"
curl "http://localhost:3010/api/online/play?id=歌曲ID"
```
...
EOF
 ...
 echo -e "${GREEN}[√] Hermes Skill 配置已创建${NC}"
 fi
}

# 创建 OpenClaw Skill 配置
create_openclaw_skill() {
 SKILL_DIR="$HOME/.openclaw/skills/web-audio-streamer"
 
 if [ -d "$HOME/.openclaw" ]; then
 echo ""
 echo -e "${BLUE}[→] 创建 OpenClaw Skill 配置...${NC}"
 mkdir -p "$SKILL_DIR"
...
EOF
 ...
 echo -e "${GREEN}[√] OpenClaw Skill 配置已创建${NC}"
 fi
}
...
start_server() {
 ...
 echo -e "${GREEN}=========================================${NC}"
 echo ""
 echo -e " 访问地址: ${BLUE}http://localhost:3010${NC}"
 echo ""
 ...
 *)
 ...
 echo ""
 echo "或使用启动脚本（自动检查依赖）:"
 echo " cd $INSTALL_DIR && npm run start:check"
 fi
}
